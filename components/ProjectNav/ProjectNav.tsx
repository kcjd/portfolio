...
const ProjectNavLink = ({ project, isPrevious, isNext }: ProjectNavLinkProps) => {
  ...
  return (
    <Link href={`/project/${project.slug}`} passHref>
      ...
      >
        ...
        >
          <Image src={`/projects/${project.thumbnail}`} width={1500} height={1200} layout="responsive" alt="" />
        </motion.div>
        ...
        >
          <p className="text-lg text-gray-600">{isPrevious ? 'Précédent' : 'Suivant'}</p>
...
const ProjectNav = ({ previousProject, nextProject }: ProjectNavProps) => {
  return (
    <section className="container flex flex-col md:flex-row gap-16 py-12">
      {previousProject && <ProjectNavLink project={previousProject} isPrevious />}
